...
date_month = datetime.now(tz).strftime("%Y%m")
...
def get_all_parameter():
    ...
    df_all = pd.DataFrame(data, columns=['code', 'date', 'close'])
    ...
    for code in tqdm(codes):
        ...
        if len(df)>1000:
            ...
            with get_connection() as cursor:
                sql = '''
                replace into etf.ads_nasdaq_strategy values (%s, %s, %s, %s, %s)
                '''
                cursor.execute(sql, (code, date_month, annu_returns, sharpe, str(best_parameter)))
