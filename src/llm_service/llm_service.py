...
app = FastAPI()
client_max = Client("Qwen/Qwen-Max-0428")
client_110b = Client("Qwen/Qwen1.5-110B-Chat-demo")
client_72b = Client("Qwen/Qwen1.5-72B-Chat")
...
async def list_models():
    ...
    return ModelList(data=[model_card,
                           ModelCard(id='Qwen-Max-0428'),
                           ModelCard(id='Qwen1.5-110B-Chat'),
                           ModelCard(id='Qwen1.5-72B-Chat'),
                           ])
...
def choose_model(model):
    if model == "Qwen-Max-0428":
        return client_max
    
    if model == "Qwen1.5-110B-Chat":
        return client_110b
...
async def create_chat_completion(request: ChatCompletionRequest):
    logger.info(str({"data": dict(request.model_dump())})+"\n")
    ...
    query, history, system = parse_messages(request.messages,
                                            request.functions)
    logger.info(str({"query":query, "history":history, "system": system})+"\n")
    if request.stream:
        if request.functions:
            raise HTTPException(
                status_code=400,
        ...
        client = choose_model(request.model)
        ...
        choice_data = ChatCompletionResponseChoice(
            index=0,
            message=ChatMessage(role='assistant', content=result[1][-1][1]),
            finish_reason='stop',
        )
...
async def predict(
    ...
    new_query = ""
    for user_input, assistant_input in history:
        new_query = new_query + "\n" + user_input + "\n"+ assistant_input
    new_query = new_query + query
    client = choose_model(model_id)
    response_generator = client.submit(
                query=new_query[-max_token:],
                history=[],
                system=system,
                api_name="/model_chat"
        )
    for _new_response in response_generator:
        _new_response = _new_response[1][-1][1]
        ...
        chunk = ChatCompletionResponse(model=model_id,
                                       ...
                                       object='chat.completion.chunk')
        yield '{}'.format(_dump_json(chunk, exclude_unset=True))
    logger.info({"_new_response": _new_response})
...
if __name__ == '__main__':
    args = _get_args()
    uvicorn.run("llm_service:app", host=args.server_name, port=args.server_port, workers=1)
